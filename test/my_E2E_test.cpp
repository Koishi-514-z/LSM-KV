...
private:
    ...
    void text_test(uint64_t max) {
        ...
        for (i = 0; i < max; ++i) {
            ...
            for(auto j : res) {
                bool match = (ans[idx] == j.second);
                if (!match) {
                    error_log << "\nMISMATCH at idx=" << idx << std::endl;
                    error_log << "Expected: " << ans[idx].substr(0, 100) << "..." << std::endl;
                    error_log << "Actual: " << j.second.substr(0, 100) << "..." << std::endl;
                }
